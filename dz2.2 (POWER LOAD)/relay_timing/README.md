...
Підсумок тієї ж серії:

| | avg | min | max |
|---|---|---|---|
| увімкнення (pull-in) | 4141 us (4.14 ms) | 4120 us | 4159 us |
| брязкіт при увімкненні | 227 us (0.22 ms) | 222 us | 229 us |
| вимкнення (release) | 3562 us (3.56 ms) | 3542 us | 3579 us |
| брязкіт при вимкненні | 13 us (0.01 ms) | 0 us | 30 us |

Фронтів: 144 на 15 увімкнень (9.6 на спрацювання) і 22 на 15 вимкнень (1.4).

У самому логу ці рядки з'їхали, і причина повчальна: `printf` вирівнює `%-26s` по **байтах**, а кирилиця в UTF-8 займає два байти на літеру. Мітка з 20 символів важить 30 байтів, формат вважає її задовгою і не додає жодного пробілу. Полагодив окремою функцією, яка рахує символи за старшими бітами: у продовження символу UTF-8 вони завжди `10`, у початку — ні.
...
[`analysis/serial.log`](analysis/serial.log) лишився таким, як його надрукувала плата до цієї правки.
...
**Увімкнення: ΔT = 4.1113 мс**, одразу за фронтом контакту — пачка брязкоту.

<a href="analysis/pull-in.png"><img src="analysis/pull-in.png" alt="Захоплення логічного аналізатора: від фронту команди до замикання контакту 4.111 мс, далі пачка брязкоту"></a>

**Вимкнення: ΔT = 3.5701 мс**, один чистий фронт без жодного дзвону.

<a href="analysis/release.png"><img src="analysis/release.png" alt="Захоплення логічного аналізатора: від фронту команди до розмикання контакту 3.570 мс, один чистий фронт"></a>

Картинки клікабельні — відкриваються в повній роздільності.
...
| Підсумок за 15 циклів у Wokwi | avg | min | max |
|---|---|---|---|
| увімкнення (pull-in) | 5014 us (5.01 ms) | 5009 us | 5022 us |
| брязкіт при увімкненні | 0 us | 0 us | 0 us |
| вимкнення (release) | 5022 us (5.02 ms) | 5022 us | 5022 us |
| брязкіт при вимкненні | 0 us | 0 us | 0 us |

Фронтів при увімкненні: 15 на 15 вимірів, тобто рівно один на спрацювання.
